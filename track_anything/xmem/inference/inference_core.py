...
class InferenceCore:
    ...
    def step(self, image, mask=None, valid_labels=None, end=False):
        ...
        is_deep_update = (
            (self.deep_update_sync and is_mem_frame) or (not self.deep_update_sync and self.curr_ti - self.last_deep_update_ti >= self.deep_update_every)  # synchronized  # no-sync
        ) and (not end)
        ...
        if need_segment:
            ...
            hidden, pred_logits_with_bg, pred_prob_with_bg = self.network.segment(multi_scale_features, memory_readout, self.memory.get_hidden(), h_out=is_normal_update, strip_bg=False)
            # remove batch dim
        ...
        if is_mem_frame:
            value, hidden = self.network.encode_value(image, f16, self.memory.get_hidden(), pred_prob_with_bg[1:].unsqueeze(0), is_deep_update=is_deep_update)
