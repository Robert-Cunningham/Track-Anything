...
class XMemTrainer:
    def __init__(self, config, logger=None, save_path=None, local_rank=0, world_size=1):
        ...
        self.XMem = nn.parallel.DistributedDataParallel(XMem(config).cuda(), device_ids=[local_rank], output_device=local_rank, broadcast_buffers=False)
        ...
        self.train()
        self.optimizer = optim.AdamW(filter(lambda p: p.requires_grad, self.XMem.parameters()), lr=config["lr"], weight_decay=config["weight_decay"])
    ...
    def save_checkpoint(self, it):
        ...
        checkpoint = {"it": it, "network": self.XMem.module.state_dict(), "optimizer": self.optimizer.state_dict(), "scheduler": self.scheduler.state_dict()}
