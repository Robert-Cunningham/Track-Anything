...
class BaseSegmenter:
    ...
    def predict(self, prompts, mode, multimask=True):
        ...
        if mode == "point":
            ...
            masks, scores, logits = self.predictor.predict(point_coords=prompts["point_coords"], point_labels=prompts["point_labels"], mask_input=prompts["mask_input"], multimask_output=multimask)
        else:
...
if __name__ == "__main__":
    ...
    prompts = {"point_coords": np.array([[500, 375], [1125, 625]]), "point_labels": np.array([1, 0]), "mask_input": mask_input[None, :, :]}
